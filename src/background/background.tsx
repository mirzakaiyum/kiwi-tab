...
function getBackgroundIndex(): number {
    ...
    if (savedIndex === -1 || lastShuffle === 0 || (frequency !== "never" && now - lastShuffle >= getFrequencyMs(frequency))) {
    ...
    if (currentBg) {
        const typeMismatch = (type === "images" && currentBg.type !== "image") || 
                             (type === "videos" && currentBg.type !== "video");
        if (typeMismatch) {
            const validIndices = getValidIndices();
            if (validIndices.length > 0) {
                 const randomChoice = Math.floor(Math.random() * validIndices.length);
...
function BackgroundLayer({ data, onLoad, onError, zIndex }: BackgroundLayerProps) {
    const { src: cachedUrl } = useCachedMedia(data.url, { enabled: data.type === "video" });
...
export function Background() {
    ...
    const handleLayerError = useCallback((id: number) => {
        ...
        setLayers(prev => {
            ...
            // But we mark it loaded so opacity sets to 1 (at least show something vs invisible).
             return prev.map(layer => layer.id === id ? { ...layer, loaded: true } : layer);
        });
    }, []);
    ...
    if (backgroundType === "none") return <div className="fixed inset-0 -z-10 bg-background" />;
